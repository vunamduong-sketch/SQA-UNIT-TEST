...
// ============================================================
// TÊN FILE: activityCityCoverageTable.js
// MÔ TẢ: Custom Jest coverage reporter cho folder ActivityCity.
//        Reporter in tổng quan Total/Statements/Miss và bảng chi tiết
//        File | Statements | Miss | Coverage giống mẫu báo cáo.
// ============================================================

class ActivityCityCoverageTableReporter {
  onRunComplete(_, results) {
    ...
    const componentDir = path.normalize(
      path.join(process.cwd(), "src-under-test", "components", "ActivityCity")
    );
    ...
    if (rows.length === 0) {
      console.log("Không tìm thấy coverage cho src-under-test/components/ActivityCity.");
      return;
    }
    ...
    console.log("\nActivityCity Coverage Summary");
    console.table(rows);
  }
}

module.exports = ActivityCityCoverageTableReporter;
