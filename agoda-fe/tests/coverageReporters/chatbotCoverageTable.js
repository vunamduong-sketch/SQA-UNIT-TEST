...
// ============================================================
// TÊN FILE: chatbotCoverageTable.js
// MÔ TẢ: Custom Jest coverage reporter cho folder Chatbot.
//        Reporter in tổng quan Total/Statements/Miss và bảng chi tiết
//        File | Statements | Miss | Coverage giống mẫu báo cáo.
// ============================================================

class ChatbotCoverageTableReporter {
  onRunComplete(_, results) {
    ...
    const chatbotDir = path.normalize(
      path.join(process.cwd(), "src-under-test", "components", "Chatbot")
    );
    ...
    coverageMap.files()
      .filter((filePath) => path.normalize(filePath).startsWith(chatbotDir))
    ...
    if (rows.length === 0) {
      console.log("Không tìm thấy coverage cho src-under-test/components/Chatbot.");
      return;
    }
    ...
    console.log("\nChatbot Coverage Summary");
    console.table(rows);
  }
}

module.exports = ChatbotCoverageTableReporter;
